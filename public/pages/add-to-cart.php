...
//ici ca check si le film est déjà acheté
if (hasUserPurchasedMovie($_SESSION['user_id'], $movieId)) {
    setFlashMessage('Vous avez déjà acheté "' . $movie['title'] . '". Ce film est disponible dans votre bibliothèque.', 'error');
    $redirect = isset($_GET['redirect']) ? $_GET['redirect'] : (isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : SITE_URL . '/profile.php');
    redirectTo($redirect);
}
